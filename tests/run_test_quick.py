#!/usr/bin/env python3
"""
Quick test suite for simulation pipeline.

Tests only Llama-3.1-8B configurations with 1 user and 5 responses.
...
def run_test(data_file: str, output_dir: str = "test_results", n_users: int = 1, n_responses: int = 5):
    """
    Run quick test with Llama-3.1-8B only.
    ...
    print("=" * 80)
    print("QUICK TEST SUITE - Llama-3.1-8B Only")
    print("=" * 80)
    ...
    if failed_configs:
        ...
        for fail in failed_configs:
            print(f"  - {fail['config']}: {fail.get('error', 'Unknown error')}")

    # Save test report
    report_path = os.path.join(output_dir, "quick_test_report.json")
    report = {
        "test_type": "quick",
        "seed": seed,
        "n_users": n_users,
...
def main():
    parser = argparse.ArgumentParser(
        description="Quick test suite for simulation (Llama-3.1-8B only)"
    )
    parser.add_argument(
        "--data_file",
        type=str,
