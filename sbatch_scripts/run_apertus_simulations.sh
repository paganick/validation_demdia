#!/bin/bash
#SBATCH --job-name=apertus_sim
#SBATCH --output=logs/apertus_%A_%a.out
#SBATCH --error=logs/apertus_%A_%a.err
...
# Run simulations for Apertus configs across all datasets
# Total: 5 configs × 3 datasets = 15 jobs
...
# Get list of Apertus configs (non-finetuned only for now)
mapfile -t CONFIGS < <(ls configs/*apertus*.yaml | grep -v finetuned | sort)
