#!/usr/bin/env python3
"""
Configuration optimization analysis script (CONSOLIDATED FORMAT VERSION).
Generates 7 key figures from the new consolidated postprocessing format:
...
# Import consolidated data loaders
from consolidated_data_loader import (
    load_configuration_data_consolidated,
    load_cosine_similarities_by_response_type_consolidated,
    analyze_response_overlap_consolidated,
    load_empath_feature_data_consolidated,
    load_feature_importance_data_consolidated
)
...
def main(folder="results"):
    ...
    Config.OUTPUT_DIR = f"{folder}/configuration_optimization_figures_consolidated"
    ...
    # Load accuracy data from CONSOLIDATED format
    print("\n=== Loading Accuracy Data (Consolidated Format) ===")
    df = load_configuration_data_consolidated(Config.DATASETS)
    ...
    # Analyze and plot response overlap from CONSOLIDATED format
    df_overlap = analyze_response_overlap_consolidated(Config.DATASETS)
    ...
    # Load cosine similarity data from CONSOLIDATED format
    similarity_data = load_cosine_similarities_by_response_type_consolidated(Config.DATASETS)
    ...
    # Generate Empath feature frequency plot from CONSOLIDATED format
    feature_model_counts = load_empath_feature_data_consolidated(Config.DATASETS, df_best_configs, response_type='random')
    if feature_model_counts:
        plot_empath_feature_frequency(Config.DATASETS, df_best_configs, top_n=20)

    # Generate feature importance heatmap from CONSOLIDATED format
    df_importance = load_feature_importance_data_consolidated(Config.DATASETS, df_best_configs, response_type='random')
